...
function preStepGeneralDistributionRN(data, counters, globalParams, U_init)
	...
	for i_upper_lower in 1:2
		...
		function cHat_baseIndex_solver_func!(cHat_x)
			return gmFunction_baseIndex!(sigma, tau, UPow_init, wHat, vcat(ones(1), cHat_x[1:D-1]), cHat_x[D:D+2], cHat_x[D+3], lambda, baseIndex, UoModel_factor, upper_lower[i_upper_lower], 1, RN, κ_Frechet, thetaHat) # we force the cHat[1,:] =1
		end
		cHat_solver_results = nlsolve(cHat_baseIndex_solver_func!, vcat((cHat_Frechet[2:D, baseIndex]) .^ (1.0 / thetaHat), 0.5, 1, 1, 1), ftol = 1e-3 ) #, autodiff = :forward
		...
		cHat[2:D, baseIndex] .= abs.(cHat_solver_results.zero)[1:D-1]
		...
		for d ∈ 1:D
			if d != baseIndex
				function cHat_solver_func!(cHat_x)
					return gmFunction_d!(sigma, tau, UPow_init, wHat, vcat(ones(1), cHat_x[1:D-1]), lambda, d, UoModel_factor, RN) # we force the cHat[1,:] =1
				end
				cHat_solver_results = nlsolve(cHat_solver_func!, (cHat_Frechet[2:D, d]) .^ (1.0 / thetaHat)) #, autodiff = :forward
				@show cHat_solver_results.residual_norm ## check it converged
				cHat[2:D, d] .= abs.(cHat_solver_results.zero)[:]
...
@show gammaHat[:,2]
	...
	output = (μHat = 1 ./ thetaHat,
		...
		γHat_upper = copy(gammaHat[:, 1]),
		γPrimeHat_upper = copy(gammaPrimeHat[:, 1]),
		Aod_initial_upper = reshape(Aod_initial[:, :, 1], D^2)[:],
		wPrimeHat_lower = copy(wPrimeHatFrechet[:]),
		γHat_lower = copy(gammaHat[:, 2]),
		γPrimeHat_lower = copy(gammaPrimeHat[:, 2]),
		Aod_initial_lower = reshape(Aod_initial[:, :, 2], D^2)[:],
	)
...
function gmFunction_baseIndex!(σ, τ, UPow_init, w_, cPow_, α_m_, K, lambda, d, UoModel_factor, lower_bound, δ_max, m, κ_Frechet, thetaHat)
	# encountered issues as solver was trying negative values so used abs. maybe there is a better solution
	w = abs.(w_)
	cPow = abs.(cPow_)
	α_m = abs.(α_m_)
	...
	for ω ∈ 1:W
		...
		m[ω] = α_m[1] * SmoothOneSidedDirac(0.01, -(V-V_rw)) + 1- α_m[1]
		dirac_exp2 += SmoothOneSidedDirac(0.01, (U_pow-U_rw_pow))/W
		dirac_exp3 += SmoothOneSidedDirac(0.01, -(V-V_rw))/W
		m_expectation += m[ω] / W
...
function gmFunction_d!(σ, τ, UPow_init, w_, cPow_, lambda, d, UoModel_factor, RN)
	# encountered issues as solver was trying negative values so used abs. maybe there is a better solution
	w = abs.(w_)
	cPow = abs.(cPow_)
