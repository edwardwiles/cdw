...
LOG=sequential_gravity/coldstart_delta1_W80000_run.log
echo "=== Launch: $(date) ===" > "$LOG"

# Separate OUT_DIR: checkpoint filenames don't encode W, so this must not share a
# directory with the W=8000 runs or it would silently overwrite/collide with them.
FAKEDATA=3 DVAL=20 DELTA_GRID=1.0 BOUND=upper PARALLEL_INVERSION=true WVAL=80000 \
  ...
  OUT_DIR=$PERF/sequential_gravity/batch_out_realD20_W80000 \
...
touch sequential_gravity/coldstart_delta1_W80000_ALLDONE
