...
function precalcCDFs(Ū, params, prestep_output)
    ...
    CDF_Moments = zeros(W, 2 * momentOrder * D^2 + 2 * momentOrderForBaseIndex * D + 2 * D^2) # for each od: I. for each CDF point, i) one truncated moment + ii) CDF + II. first moment and (μHat * (1 - σHat)) moment 
