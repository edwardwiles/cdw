import Pkg
Pkg.activate("/bbkinghome/edav/gravity_robustness/trade_robustness_modular")
...
using KNITRO
using Random, NLsolve, SpecialFunctions, DelimitedFiles, Dates, LinearAlgebra, Parameters
using Distributions, Statistics
using ForwardDiff, Calculus   # replaces the missing CounterfactualSensitivity_local.jl (Main-scope imports for the bundle/loop code)
# NOTE (run copy): dropped off-default-path deps Optim, Combinatorics, Bigsimr, Plots, InvertedIndices, MemPool
#so code doesn't throw error messages as it tries to display figures.
ENV["GKSwstype"] = "nul"

include("../cc_algo/knitro_compat.jl")   # restore KNITRO.jl 0.13/0.14 convenience wrappers on v1.2.1
include("softmax.jl")
include("Psi.jl")
...
function setwd(server, user)
    ...
    if server == 1 # if working on the econ servers
        ...
        if user == 2
            # run copy: cd into the working repo root so the .opt files (loaded by bare
            # filename by the KNITRO loops) and output CSVs resolve here.
...
function buildObjectsForMoments(globParams, preStepOutput, data, LPrime, τPrime, Uσ, Ū, Σ_od, Mτ, μHat, PMM=zeros(1), CDF_X=zeros(1), CDF_Moments=zeros(1), Ind_Moments=zeros(1), IndCDF_Cells =Vector{Vector{Int}}(undef,1), importanceSamplingWeights = ones(1) )
...
function runMainClosedFormeFrechet(globParams)
    ...
    if independenceMoment == 1 && NoScalingforSameMartingale == 1 && useIndependentCFDs == 0
        offset = gravMoment + localGravityMoment * (D - 1) * D + localGravityCrossMoment * D * (D - 1) * (D - 2) + GravityMomentFirstApproach * (3 + D^2) + sameMarginalsMoment * (2 * momentOrder) * D^2
    ...
    if independenceMoment == 1 && NoScalingforSameMartingale == 1&& useIndependentCFDs == 1
    ...
    γ, θ_initial = buildObjectsForMoments(globParams, preStepOutput, data, LPrime, τPrime, Uσ, Ū, Σ_od, Mτ, μHat, PMM, CDF_X, CDF_Moments, Ind_Moments, IndCDF_Cells, ImportanceSamplingWeight)
