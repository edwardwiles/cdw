...
#
# Adapted 2026-07-30 for the sigma3/W500k five-family production campaign: REAL_DATA_DIR points at
# the immutable snapshot, W=500_000, MEANZC_K=ORIGINZC_K=2, exclude_diagonal_gravity=true,
# sigmaHat=3.0, and the 5-starts manifest check is relaxed to accept this campaign's 3-start
# manifest (start_manifest.json).
const _D4E = normpath(joinpath(@__DIR__, "..", "..", "..", "full_aod_diag", "d4_exact"))
ENV["REAL_DATA_DIR"] = normpath(joinpath(@__DIR__, "..", "data_snapshot"))
...
length(all_starts) == 3 || error("manifest at $MANIFEST_PATH does not have exactly 3 starts (has $(length(all_starts)))")
    ...
    try
        if FAMILY == "origin_zc"
            ...
            result = fn(w0; W = W, delta = delta, draw_design = DRAW_DESIGN, draw_seed = DRAW_SEED,
                ...
                ckpt_dir = ckdir, run_id = label, label = label,
                checkpoint_interval_s = 3600.0, maxtime_real = MAXTIME, verbose = true,
                exclude_diagonal_gravity = true, σHat = 3.0, extra...)
        end
    catch e
        errored = true
