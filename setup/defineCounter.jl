function defineCounter(τData, LData, counterType)
    ...
    elseif counterType == 1 # autarky
        ...
        # with τData==1 exactly (a real occurrence in real trade-cost data -- never arises in the
        # synthetic fakeData generators, which is why this was never caught before) would silently
        # stay fully tradable under "autarky" instead of becoming infinitely costly. Construct
        # τPrime directly instead: Inf everywhere off-diagonal, own-country cost unchanged.
        # Ported from trade_robustness_modular_perf @ 778c362 (feature/sequential-inversion-perf).
        τPrime = fill(Inf, D, D)
        for i in 1:D
