...
3. **Live smoke test** (`phase8_transformed_a_default_smoke.jl`, new this session): real D=20/
   ...
   - Common Fréchet: PASS
   - CM+ZC: PASS
   - Origin-ZC: *(this session's first two attempts hit real script bugs — wrong include order for
     `cm_originzc_config.jl`/`cm_originzc_target_layout.jl`, and an invalid
     `distribution_restriction` symbol value — both fixed; result appended once the corrected run
     completes.)*
...
cm_plus_zc=promoted+verified, zc_only=promoted, live-smoke-pending-rerun (see above).
