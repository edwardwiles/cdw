# Full-D Melitz Δ\* benchmark: design document (SUPERSEDED, ARCHIVED)

> **This document describes the SUPERSEDED f_entry-primitive / N-derived / N'=N closure.**
> It is preserved for history only (addendum Section 16). The ACTIVE closure (baseline
> `N_o=1`, searched `gamma_prime_j`, derived `f[j,j]`, `D^2+1` moments, LFD recovery) is
> documented in `docs/melitz_delta_star.md`. Do not use this file to understand the
> current code -- see that file's "Superseded statements" section for the itemized list of
