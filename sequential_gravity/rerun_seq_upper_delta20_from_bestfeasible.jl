...
# ============================================================================
# Targeted rerun: delta=20 upper, warm-started from delta=10's BEST-FEASIBLE
# theta (not KNITRO's own terminal theta, which was degenerate: R_mean=1.1e+107,
# gravity-infeasible). Chaining delta=20 from that degenerate point meant the
# delta=20 search never found a single feasible candidate the whole run
# ("best-feasible: NONE FOUND"). Using the KNOWN-GOOD delta=10 best-feasible
# point as delta=20's starting point instead.
# ============================================================================
const OUT_DIR = joinpath(@__DIR__, "batch_out")
d10 = JLD2.load(joinpath(OUT_DIR, "seq_upper_delta10.0.jld2"))
θinit = d10["best_feasible_theta"]
@assert θinit !== nothing "delta=10 has no best_feasible_theta to warm-start from"
println(">>> warm-starting delta=20 upper from delta=10's best-feasible theta (kappa=", d10["best_feasible_kappa"], ")")

r = run_one_bound(:upper, true, 20.0, θinit)
path = joinpath(OUT_DIR, "seq_upper_delta20.0.jld2")
JLD2.save(path, Dict(
    "method" => "sequential_profiled", "bound" => "upper", "delta" => 20.0, "D" => D,
    ...
    "starting_point_source" => "warm-started from delta=10's BEST-FEASIBLE theta (KNITRO-own was degenerate)",
    "kappa_point_estimate" => KAPPA_POINT_EST,
    "done" => true))
println("RERUN_SEQ_UPPER_DELTA20_DONE")
