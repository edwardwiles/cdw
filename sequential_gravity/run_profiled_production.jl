...
function run_at_delta(δval::Real)
    @printf("\n=== [PRODUCTION: profiled/sequential, cached, free-only ForwardDiff] D=%d W=%d δ=%g ρ=%g MU_FIXED(removed)=%s ===\n",
    ...
    for (name, fs) in ((:lower, false), (:upper, true))
        ...
        t0 = time()
        gp, θstar, st, bθ, b_gp, bwarm, cache = outer_solve_nested_cached(fs, θr0; use_exact_grad = true, δ = δval)
        κ = gp2kappa(gp)
        ...
        if bθ === nothing
            ...
            results[name] = (κ = κ, gp = gp, R = Rθ, ok = okθ, best_κ = NaN, best_gp = NaN, best_θ = nothing, best_ok = false, cache = cache)
        else
            ...
            results[name] = (κ = κ, gp = gp, R = Rθ, ok = okθ, best_κ = bκ, best_gp = b_gp, best_θ = bθ, best_ok = okb, cache = cache)
        end
    end

    @printf("\n=== PRODUCTION RESULTS (delta=%g) ===\n", δval)
