# γ_focal≡1 NORMALIZED variant of run_profiled_bounds.jl. Same §18 nested profiled-gravity method,
# but reparameterized per review: γ_focal is FORCED to 1 (not a free parameter) and γ'_focal enters
# DIRECTLY (not via a raw _θ multiplier) with THEORETICAL bounds [λ_dd^(1/σ), 1] — the exact,
# derived-and-verified range implied by 0 ≤ κ ≤ 1-λ_dd^(1/(σ-1)) (see focal_moments.jl's
# EK_moments_focal_norm! docstring for the full derivation/verification against computeGamma.jl).
# γ_focal≡1 REPLACES (not adds to) the A[1,focal]=1 normalization — the two are the same gauge-fix
# for the same redundancy (a common Acol scale is exactly redundant with γ_focal's level), so ALL D
# entries of Acol are free here, starting from the scale s=γf0^(-σ/(μ(σ-1))) that keeps θ_norm's
# starting point at (near-)zero divergence from F* (verified: actual δ* needed ≈1.8e-4).
# Structural motivation: this bounds κ into its valid range directly via θ-bounds (provably correct,
# GT∈[0,κ_max] — see the paper draft), rather than only catching violations after the fact via the
# divergence-budget check in seq_gravcol (which remains active here too, as a second line of defense).
#
#   julia --project=. sequential_gravity/run_profiled_bounds_norm.jl      (needs KNITRO env)
...
# NORMALIZED starting point: γ'_focal(direct) = γ'_focal/γ_focal (original ratio, preserves κ
# exactly); Acol = s·[1,...,1] with s=γf0^(-σ/(μ(σ-1))) (the exact compensating common scale that
# keeps θ_norm's moments at ~zero divergence from F* — verified δ*≈1.8e-4, see commit message).
...
const KBOUNDS = theoretical_kappa_bounds(γ, σ)   # (κ_min=0, κ_max, γp_lo, γp_hi)
...
function seq_gravcol(θ; δ::Real = δ, maxit = 20, tol = 5e-4, warm = nothing, verbose = false)
    ...
    p, ok = recover_lfd(θ, EK_moments_focal_norm!, D + 1)
    if !ok
    ...
    for k in 1:maxit
        ...
        moments_aug! = (K, G, θθ, Uarg, obj) -> begin
            EK_moments_focal_norm!(K, @view(G[:, 1:D+1]), θθ, Uarg, obj); @. G[:, D+2] = infl.ψ_bar + infl.R_beta
        end
...
function focal_bounds(θr)
    ...
    lo[1] = 0.001; hi[1] = 1/(σ-1) - 0.001                    # μ range
    lo[3] = KBOUNDS.γp_lo; hi[3] = KBOUNDS.γp_hi              # γ'_focal: THEORETICAL bounds (structural fix)
    # Acol (indices 4:3+D) ALL free — no A[1,focal]=1 pin: γ_focal≡1 already fixes the
    # common-scale gauge, so pinning A[1,focal] too would double-constrain the same redundancy.
    if FREEZE_MU
        lo[1] = θr[1]; hi[1] = θr[1]
    end
    lo, hi
...
function make_stateful_moments(; use_exact_grad::Bool = true, find_smallest::Bool = false, δ::Real = δ)
    ...
    function m!(K, G, θ, Uarg, obj)
        if !(eltype(θ) <: ForwardDiff.Dual)          # value path: (re)linearize gravity at this θ
            ...
            if θf != lastθ[]
                ...
                if ok
                    ...
                    EK_moments_focal_norm!(Ktmp, Gtmp, θf, view(U, 1:1, :), (γ = γ,))
                    κθ = Ktmp[1]
        ...
        EK_moments_focal_norm!(K, @view(G[:, 1:D+1]), θ, Uarg, obj)
        nrow = size(G, 1)
...
function outer_solve_nested(find_smallest, θinit; use_exact_grad::Bool = true, δ::Real = δ)
    ...
    obj = PsiObjectiveBundleImplicit(δ = δ, find_smallest = find_smallest, γ = γ,
        ...
        outer_loop_opt = get(ENV, "OUTER_OPT_FILE", "csw_outer_loop_settings_cluster.opt"),
...
Kchk = zeros(W); Gchk = zeros(W, D + 1); EK_moments_focal_norm!(Kchk, Gchk, θr0, U, (γ = γ,))
POINT_EST = Kchk[1]
