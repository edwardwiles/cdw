# ================================================================================
# Third variant: same γ_d≡1-for-all-d gauge as compare_gammanorm.jl, but the
# KNITRO objective is γ'_focal DIRECTLY (EK_moments_gammanorm_directgp!) instead
# of the power-transformed κ=1-(γ'/γ)^{σ/(σ-1)}. Since κ is a strictly monotone
# (decreasing) transform of γ'_focal alone, extremizing γ'_focal and converting
# afterward gives identical bounds in principle — this tests whether the power
# transform itself was adding conditioning trouble on top of the A[1,d]=1-vs-
# γ_d≡1 gauge choice. Same economy/data/draws, same θ bounds/init as
# compare_gammanorm.jl's "gammad_norm" variant; maxit=25 for apples-to-apples.
#
# SIGN CONVENTION: κ decreasing in γ'_focal ⇒ find_smallest must be SWAPPED
# relative to the κ-objective runs to get the same κ bound, then convert back.
#
# Run: julia --project=. full_aod_diag/compare_directgp.jl   (needs KNITRO env)
# ================================================================================
...
# ---------------- θ bounds/init: identical to compare_gammanorm.jl's "gammad_norm" variant ----------------
...
println(">>> VARIANT=directgp  free params=$nfree  (same θ layout/bounds as gammad_norm)"); flush(stdout)
...
t0=time()
# κ_lower needs γ'_focal MAXIMIZED (κ decreasing in γ') -> find_smallest=false in the
# original κ-objective convention -> pass find_smallest=false here to directly ask
# outer_loop for max(γ'_focal); it returns the (sign-corrected) MAX value.
println(">>> directgp: solving for max(gamma'_focal) [-> kappa_lower]"); flush(stdout)
gp_max, θ_lo, st_lo, _ = outer_loop(mkobj(false, θ_initial_low_G), θ_lower_G, θ_upper_G, θ_initial_low_G)
println(">>> directgp: solving for min(gamma'_focal) [-> kappa_upper]"); flush(stdout)
...
open(joinpath(OUT,"compare_directgp_summary.txt"),"w") do f
    for ln in (
        "variant=directgp  D=$D  baseIndex=$bi  free=$nfree  maxit=25",
...
@save joinpath(OUT,"compare_directgp.jld2") κ_up κ_lo gp_max gp_min θ_up θ_lo st_up st_lo
println("\n=== directgp vs gammad_norm (both maxit=25) ===")
@printf("directgp     kappa=[%.6f, %.6f] width=%.6f  status=(%d,%d)  wall=%.1fs\n",
    κ_lo, κ_up, κ_up-κ_lo, st_lo, st_up, wall)
println("DONE compare_directgp")
