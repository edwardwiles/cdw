...
# GENUINE-COLD common-Frechet solve at PRODUCTION dimensions (D=20, Ddest=19, L=50, W=100,000), via
# the zero-dense reduced/operator inner-solve path (`reduced_frechet_base_state`). Must be launched
...
println("="^90); println("GENUINE-COLD common-Frechet solve, PRODUCTION DIMS: D=$D_VAL Ddest=$DDEST_VAL L=$L_VAL W=$W_VAL")
...
t_build = @elapsed begin
    aug_reduced = build_cm_frechet_augmented_obj_archB(ctx, CS; L = L_VAL, contrasts = :anchored, base_obj = reduced_obj0, profiled_layout = layout)
...
t_solve = @elapsed base = reduced_frechet_base_state(x_free_calib, ctx, layout, cctx_reduced, level_targets)
...
# full writeup): verify_inner_solution_operator_cm_frechet! hardcodes `ncore1 = cf.oci-1` as the
# economic block width, which only matches lambda's actual economic-block width on the FULL/dense
# path -- on this REDUCED path that width is layout.total_reduced_economic_moments, a smaller
# number. Use the direct brute_force_verify (obj.moments!/Psi!/dPsi!) instead.
