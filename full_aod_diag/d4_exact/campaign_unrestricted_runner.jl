...
mkpath(OUTROOT)
outer_csv = joinpath(OUTROOT, "unrestricted_$(DIRECTION)_outer_log.csv")
...
inner_csv = joinpath(OUTROOT, "unrestricted_$(DIRECTION)_inner_log.csv")
open(inner_csv, "w") do io
    println(io, "family,direction,delta,start,eval_idx,t_s,Delta_star,inner_status,delta_feasible,classification_proxy,final_incumbent")
    for r in rows_inner
        println(io, r.family, ",", r.direction, ",", r.delta, ",", r.start, ",", r.eval_idx, ",", r.t_s, ",",
                r.Delta_star, ",", r.inner_status, ",", r.delta_feasible, ",", r.classification_proxy, ",",
                r.final_incumbent)
    end
end
lp(">> wrote ", outer_csv)
