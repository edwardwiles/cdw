# Task section 7, W100k tier: CM+ZC K=3 intended spec (K_mean=3, K_pair=3, meanzc_profiled_level=2)
# at real D20/W=100,000/L=50 -- calibration point + one nearby verified point, direct pcx-level
# check (not a full outer campaign, per the task's own "do not launch a full campaign" instruction).
const _D4E = @__DIR__
...
lp("="^100); lp("=== CM+ZC intended-spec (K_mean=3, K_pair=3, meanzc_profiled_level=2) D20/W=100,000/L=50 ==="); lp("="^100)
K_mean = 3; K_pair = 3
...
lp("[cmzc-k3-w100k] context built: n_families=", pcx.cctx.n_families, " Pow!==nothing=", pcx.cctx.Pow !== nothing,
...
lp("[cmzc-k3-w100k] nu_guess = ", nu_guess)

t0 = time()
base, verify = archC_meanzc_verified_state(xf0, nu_guess, pcx.ctx_cm, pcx.cctx)
t1 = time()
lp("[cmzc-k3-w100k] CALIBRATION: t=", round(t1 - t0, digits = 1), "s inner_status=", verify.inner_status,
...
# one nearby (non-calibration) point: perturb gp by a small step, matching production A-space
w1 = copy(w0); w1[1] -= 2e-4
xf1 = xf_from_w_econ(w1)
nu_guess1 = copy(nu_guess); nu_guess1[2] = meanzc_profiled_nu_value(xf1, ctx)
t2 = time()
base1, verify1 = archC_meanzc_verified_state(xf1, nu_guess1, pcx.ctx_cm, pcx.cctx)
t3 = time()
lp("[cmzc-k3-w100k] NEARBY: t=", round(t3 - t2, digits = 1), "s inner_status=", verify1.inner_status,
