# Post-merge production smoke test (2026-07-28): delta=1, flexible CM, through the real public
# checkpointed driver, real D=20/W=80,000. find_smallest passed via ARGS[1] ("true"=upper,
# "false"=lower -- confirmed convention, direction_bounds.jl). Adapted from the already-validated
# phase8_transformed_a_default_smoke.jl template (same includes/w0 construction), budget raised to
# 600s (10 min) per this smoke test's own spec.
const _D4E = @__DIR__
...
lp("=== FLEXIBLE CM smoke ($DIRECTION, find_smallest=$FIND_SMALLEST) ==="); flush(stdout)
...
result = run_cm_upper_checkpointed(copy(w_a_calib);
    ...
    cm_hessian_backend = :structured, marginal_restriction = :common_flexible, cm_gradient_backend = :cplus,
    ckpt_dir = OUT, run_id = "flexcm_$DIRECTION", label = "flexcm_$DIRECTION",
