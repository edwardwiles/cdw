...
# K_pair=3), reusing run_coldsolve_cmzc_w100k_2026-08-02.jl's exact template/bug fixes verbatim,
# only parametrizing W via ARGS[1] (defaults to 100_000) and adding a post-solve allocation
# measurement of the hot-path reduced FG functor (`base.st(x_state, g)`, same methodology as
# test_hessian_allocation_regression_2026-08-02.jl's own `@allocated`-after-JIT-warmup pattern) and
# the Hessian callback (`archC_hess_cb_builder`), so this single real cold-solve process serves
# BOTH the W100k/W500k allocation audit (item 1) and the W500k public-entry smoke (item 2).
# Must be launched as a FRESH `julia` process. Usage:
#   julia run_prodscale_cmzc_2026-08-02.jl 500000
const D4X = @__DIR__
t0_total = time()
...
# Per-level ZC nu-vector TARGET value must be k! (factorial), not flat 1.0, for K_mean>1 -- see
# run_coldsolve_cmzc_w100k_2026-08-02.jl's own header for the full writeup of this real bug.
...
g_buf = zeros(n_dual)
base.st(x_state, g_buf)  # JIT warm-up (already warm from solve, but explicit per suite convention)
...
@printf("\nTOTAL WALL: %.2fs\n", time() - t0_total)
println("CMZC_PRODSCALE_W$(W_VAL)_RESULT: ", ok ? "PASS" : "FAIL")
ok || exit(1)
