# Phase2b task (2026-08-02): combined outer-gradient decisive-PASS gate for flexible CM, following
# the ZC-lane gates' exact "complete fixed-dual FD" methodology (delta_dual_fixed_dual, adaptive
# per-coordinate bandwidth via meta.h_used) rather than test_profiled_reduced_flexcm_frechet_
# adapters_2026-08-02.jl's own `diag_profiled_full_rebuild_gradient` reference (a different,
# full-outer-rebuild FD method) -- the task explicitly asks for the ZC-lane style here.
#
# Flexible CM has NO restriction outer parameter in real production (CM marginal targets are fixed
# campaign configuration, never a live KNITRO decision variable -- confirmed by grep: there is no
# `d_delta_dual_d_eta*` function for flexible CM anywhere in this directory, unlike CM+ZC/origin-ZC
# which both have one). So this family's "combined outer gradient" is JUST the A/gp block --
# `shared_family_outer_gradient`'s own complete output, no eta/nu block appended (matching
# profiled_restricted_family_adapters_2026-08-02.jl's own header comment, Section 8).
#
# The inner solve is ALREADY genuinely zero-dense here: `evaluate_profiled_flexcm_point`
# (profiled_restricted_family_adapters_2026-08-02.jl) delegates entirely to `reduced_cm_base_state`
# (profiled_reduced_lookup_kernels_2026-08-02.jl's own `ReducedCMLookupState`/
# `inner_loop_KNITRO_reduced_cmlookup` driver) -- there is no separate "dense" gate for this family to
# diff against (unlike CM+ZC/origin-ZC, flexCM never had a dense-driver combined-outer-gradient gate
# to begin with).
const D4X = @__DIR__
for f in ["context.jl", "winners.jl", "oracle.jl", "common_marginals_moments.jl", "common_marginals_interval.jl",
          ...
          "profiled_reduced_frechet_lookup_kernels_2026-08-02.jl"]   # profiled_restricted_family_adapters_2026-08-02.jl requires BOTH bundles even though this gate only exercises flexCM
    include(joinpath(D4X, f))
end

"Inlined copy, same rationale/cross-talk-avoidance note as the ZC-lane sibling gates (this family's
own real solve never touches the unrestricted family's KNITRO driver either, but the note's own
bisection was never re-run for flexCM/Frechet -- staying consistent/conservative)."
...
"Fixed-dual Delta_dual (no nu/eta -- flexible CM has no restriction outer parameter). Rebuilds G
fresh via obj.moments! (the dense-reference closure, used ONLY as ground truth), holding
(zeta*,lambda*) fixed at the real solved point -- same discipline as the ZC-lane gates' own helper."
...
fctx = build_flexcm_family_ctx(ctx, spec, pe, layout, cctx_cm)

v = validate_family_layout_contract(fctx)
check("flexCM: layout contract validates", v.pe === pe && v.layout === layout)

before_counters = deepcopy(NO_DENSE_G_COUNTERS[])
println("="^78); println("STEP 1: TRUE ZERO-DENSE reduced flexible-CM inner solve at calibration"); println("="^78)
ev = evaluate_profiled_flexcm_point(w_profiled_calib, fctx)
check("flexCM: real KNITRO solve reaches nStatus in {0,-100,-101,-103}", ev.result.inner_status in (0, -100, -101, -103))
...
@printf("  delta dense_economic_G_materializations=%d  delta dense_CM_G_materializations=%d\n", d_econ, d_cm)
