...
# genuine-cold common-Frechet solve at PRODUCTION dimensions (D=20, Ddest=19, L=50), reusing
# run_coldsolve_frechet_w100k_2026-08-02.jl's exact template verbatim, only parametrizing W via
# ARGS[1] (defaults to 100_000) and adding a post-solve allocation measurement of the hot-path
# reduced FG functor (`base.st(x_state, g)`) and Hessian callback
# (`archC_frechet_hess_cb_builder`). Must be launched as a FRESH `julia` process. Usage:
#   julia run_prodscale_frechet_2026-08-02.jl 500000
const D4X = @__DIR__
t0_total = time()
...
b_hess = try
    cb = archC_frechet_hess_cb_builder(cctx_reduced, level_targets)
...
@printf("\nTOTAL WALL: %.2fs\n", time() - t0_total)
println("FRECHET_PRODSCALE_W$(W_VAL)_RESULT: ", ok ? "PASS" : "FAIL")
ok || exit(1)
