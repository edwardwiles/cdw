# ============================================================================
# Production-gate addendum: "Backend C+" -- combines lfix_factorized.jl's factorized
# representation (O(W*D), no price0/pTσ0 tensor) with a PERSISTENT, caller-owned workspace
# (mirroring lfix_base_workspace.jl's aliasing design for the two-tensor Backend A) so that
# repeated gradient calls at a fixed (D,W) allocate ~0 bytes for the base cache, exactly like
# Backend A, but on top of the leaner factorized arrays instead of the two big tensors.
#
# DESIGN: `LFixFactorizedWorkspace` holds every array `build_winner_ref`
# (winner_certificate.jl) + `build_lfix_base_cache_C` (lfix_factorized.jl) currently allocate
# fresh. `build_lfix_base_cache_C!(ws, x_free0, ctx, base)` refills them in place and returns a
# normal (immutable) `WinnerRefCache`/`LFixBaseCacheC` pair whose array fields ALIAS the
# workspace's buffers -- zero changes to any consumer function in lfix_factorized.jl
# (`dest_contrib_incremental_top3_C`, `count_winner_flips_C`, `select_bandwidth_C`,
# `lfix_incremental_at_C`, `composite_gradient_at_C`), exactly as Backend A's own workspace
# needed none either. Same alias-safety invariant as lfix_base_workspace.jl: no caller may
# retain an `LFixBaseCacheC`/`WinnerRefCache` returned by this builder past the NEXT
# `build_lfix_base_cache_C!` call on the same workspace (verified true of every existing
# consumer in this codebase, same audit as Backend A's own docstring).
# ============================================================================
include(joinpath(@__DIR__, "lfix_factorized.jl"))

"""
    LFixFactorizedWorkspace

Persistent backing store for Backend C+'s arrays: `WinnerRefCache`'s own (`logCC0`, `mulU`,
winner/runnerup/third indices+scores, `margin`) plus the L_fix-specific bookkeeping
(`SW`/`denom`/`CONST_d`/`contrib0`/`q0`/`Uσ_bi`/`cf_contrib0`) common to every backend. Built
ONCE per `(D,W)`, refilled on every subsequent call -- never reallocated at a fixed problem size.
"""
...
In-place-backed twin of `winner_certificate.jl::build_winner_ref`: identical formulas/order,
writes into `ws`'s persistent `logCC0`/`mulU`/winner-rank arrays instead of allocating fresh.
Returns a `WinnerRefCache` whose array fields alias `ws`'s buffers.
"""
...
"""
    build_lfix_base_cache_C!(ws, x_free0, ctx, base; validate_dense=false) -> LFixBaseCacheC

In-place-backed twin of `lfix_factorized.jl::build_lfix_base_cache_C`. Reuses `build_winner_ref!`
above for the O(W·D) ranking scan, writes `contrib0`/`q0`/cf pieces into `ws`'s own persistent
arrays. Same `ws.valid` lifecycle discipline as Backend A's `LFixBaseWorkspace`.
"""
function build_lfix_base_cache_C!(ws::LFixFactorizedWorkspace, x_free0::AbstractVector, ctx, base::BaseDualState; validate_dense::Bool = false)
    ws.valid = false
    obj = ctx.obj
...
"In-place variant of `cf_contrib_at` for `LFixBaseCacheC` (mirrors `gradient_workspace.jl::cf_contrib_at!`)."
function cf_contrib_at_C!(buf::AbstractVector, cache::LFixBaseCacheC, θ_full::AbstractVector, ctx)
...
"""
    lfix_incremental_at_Cplus!(ws::GradWorkspace, cache, ctx, pe, w0, coord_idx, new_val) -> Float64

Buffer-aware twin of `lfix_factorized.jl::lfix_incremental_at_C`, using `ws.q`/`ws.psi`/
`ws.contrib`/`ws.cf` (the SAME `GradWorkspace` slots `gradient_workspace.jl`'s two-tensor pooled
path already uses -- `price`/`pTσ` unused here, no new fields needed) instead of allocating.
"""
function lfix_incremental_at_Cplus!(ws::GradWorkspace, cache::LFixBaseCacheC, ctx, pe, w0::AbstractVector, coord_idx::Int, new_val::Float64)
    ...
    for d in affected_dests
        ...
        dest_contrib_incremental_top3_C!(ws.contrib, cache, ctx, θ_full, d, origins_here)
        ws.q .-= ws.contrib .- old_contrib
    end
    if cf_touched
        cf_contrib_at_C!(ws.cf, cache, θ_full, ctx)
...
"Buffer-aware twin of `lfix_factorized.jl::a_block_fd_component_C`, including the AUD-12 nonfinite-both-sides retry discipline."
function a_block_fd_component_Cplus!(ws::GradWorkspace, cache::LFixBaseCacheC, ctx, pe, w0::AbstractVector, coord_idx::Int, h::Float64; max_h_shrinks::Int = 4)
    h_try = h
    for attempt in 1:(max_h_shrinks + 1)
        Lp = lfix_incremental_at_Cplus!(ws, cache, ctx, pe, w0, coord_idx, w0[coord_idx] + h_try)
        Lm = lfix_incremental_at_Cplus!(ws, cache, ctx, pe, w0, coord_idx, w0[coord_idx] - h_try)
        if isfinite(Lp) && isfinite(Lm)
            return (Lp - Lm) / (2h_try)
        elseif isfinite(Lp) || isfinite(Lm)
            L0 = lfix_incremental_at_Cplus!(ws, cache, ctx, pe, w0, coord_idx, w0[coord_idx])
            isfinite(L0) || break
...
"""
    composite_gradient_at_Cplus(x_free0, ctx, pe, grad_pool::GradWorkspacePool, ws::LFixFactorizedWorkspace;
                                 base=nothing, threaded=false, h_mode=:cached, bandwidth_cache=nothing, multi_method=:top3) -> (g, meta)

"Backend C+": factorized base cache (persistent via `ws`) + the SAME per-coordinate
`GradWorkspacePool` production already uses for the buffered/pooled paths. Mirrors
`gradient_workspace.jl::composite_gradient_at_fast_pooled`'s own coordinate loop/threading
discipline exactly (including its `:static` scheduling requirement and `maxthreadid` sizing),
substituting the factorized dest_contrib/bandwidth functions from `lfix_factorized.jl` for the
two-tensor ones.
"""
function composite_gradient_at_Cplus(x_free0::AbstractVector, ctx, pe, pool::GradWorkspacePool, ws::LFixFactorizedWorkspace;
    ...
    h_mode in (:fixed, :cached) || error("composite_gradient_at_Cplus: h_mode must be :fixed|:cached, got $h_mode")
    h_mode == :cached && bandwidth_cache === nothing && error("composite_gradient_at_Cplus: h_mode=:cached requires a bandwidth_cache Dict")
