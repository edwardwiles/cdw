# Winner-aware H_ER phase (2026-07-27), Section 5: D=4 wiring-level gate for origin-ZC's H_ER
# (core x mean/pair cross block, this family's ONLY restriction block -- no CM-grid at all),
# :dense_reference vs :winner_bin, wired into archA_partitioned_hess_cb_builder
# (cm_hessian_architectures.jl). Exercises the WIRED backend end-to-end (complete packed Hessian
# via the SAME KNITRO Hessian-callback closure production uses, plus a complete real KNITRO inner
# solve), mirroring test_cm_meanzc_winner_bin_hez_wiring_d4.jl's own methodology.
const D4X = @__DIR__
...
for (K_mean, K_pair, label) in CONFIGS
    ...
    check("$label: winner_bin octx backend set", octx_wbin.zc_cross_hessian_backend === :winner_bin)

    base_dense = archOZ_base_state(x_free_calib, ν0, ctx_cm_dense)
    base_wbin = archOZ_base_state(x_free_calib, ν0, ctx_cm_wbin)
    check("$label: dense inner solve feasible", base_dense.inner_status in (0, -100, -101, -103))
    check("$label: winner_bin inner solve feasible", base_wbin.inner_status in (0, -100, -101, -103))
    check("$label: complete inner solve status matches (nStatus $(base_dense.inner_status) vs $(base_wbin.inner_status))",
        base_dense.inner_status == base_wbin.inner_status)
    check("$label: complete inner solve dual point matches",
    ...
    for (plabel, x) in (("calib", vcat(base_dense.ζstar, base_dense.λstar)),
        ...
        hd = packed_hess_via_octx(octx_dense, ctx_cm_dense.obj, x, n)
        hw = packed_hess_via_octx(octx_wbin, ctx_cm_wbin.obj, x, n)
        ...
        check("$label $plabel: complete packed Hessian dense vs winner_bin matches (max|Δ|=$(maxdiff))", ok)
        @printf("  %s %s: max|ΔH|=%.3e  scale=%.3e\n", label, plabel, maxdiff, relscale)
    end
    ...
    check("$label: zc_cross_scratch object identity stable across calls (no resize)", scratch_id_before == scratch_id_after)
    @printf("  %s: warm archA_partitioned_hess_cb_builder (winner_bin) @allocated=%d bytes\n", label, bytes)
end
