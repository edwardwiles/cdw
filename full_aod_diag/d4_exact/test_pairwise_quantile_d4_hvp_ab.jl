...
t0 = time()
nStatus_A, x_A, obj_A, n_fg_A, n_hess_A = archPQ_base_state(x_free_calib, raw_cutoffs, ctx, ctx_cm, layout)
t_A = time() - t0
...
t0 = time()
nStatus_B, x_B, obj_B, n_fg_B, n_hess_B = archPQ_base_state_hvp(x_free_calib, raw_cutoffs, ctx, ctx_cm, layout)
t_B = time() - t0
