...
include(joinpath(@__DIR__, "cm_screen_bridge.jl"))   # 2026-07-24 release fix: this script called run_originzc_upper_checkpointed (which invokes the `_screened` wrappers/counters since the 2026-07-23 screens restoration) without ever including the file that defines them -- pre-existing gap in this diagnostic script's own include list, unrelated to production entry points (originzc_production_stage_runner.jl already included this correctly)
