...
function gravity_from_logz(z::AbstractMatrix, ctx; μ::Union{Nothing,Float64} = nothing)
    ...
    return gravity_value(ctx.τ, AodPow, ctx.q_tilde, ctx.N_obs; exclude_diagonal=get(ctx, :exclude_diagonal_gravity, false), exclude_cells=get(ctx, :gravity_exclude_cells, Tuple{Int,Int}[]))
end
