# ============================================================================
# common_five_starts_search.jl  (five-family bounds campaign, 2026-07-28)
#
# Finds N_STARTS economic starting points -- the genuine calibrated point plus
...
const MAX_CANDIDATES  = length(ARGS) >= 2 ? parse(Int, ARGS[2]) : 200
const MAX_PER_RADIUS  = length(ARGS) >= 3 ? parse(Int, ARGS[3]) : 15
const OUTDIR          = length(ARGS) >= 4 ? ARGS[4] : normpath(joinpath(_D4E, "..", ".."))
...
const JSON_PATH = joinpath(OUTDIR, "COMMON_FIVE_STARTS_MANIFEST_$(STAMP).json")
const CSV_PATH  = joinpath(OUTDIR, "COMMON_FIVE_STARTS_MANIFEST_$(STAMP).csv")
const LOG_PATH  = joinpath(OUTDIR, "COMMON_FIVE_STARTS_SEARCH_LOG_$(STAMP).md")
...
lp("  start_seed=", START_SEED, " radius0=", RADIUS0, " n_starts=", N_STARTS,
...
while length(accepted) < N_STARTS && n_cand < MAX_CANDIDATES
    ...
    if rc.ok
        push!(accepted, (label = "start$(length(accepted)+1)_perturbation", radius = radius,
                         ...
                         results = rc.results))
        lp("  ==> ACCEPTED (start ", length(accepted), "/", N_STARTS, ")")
    else
...
lp()
lp("="^100)
lp("SEARCH COMPLETE: ", length(accepted), "/", N_STARTS, " common starts accepted after ",
   n_cand, " candidate evaluations in ", round(SEARCH_WALL / 60, digits = 1), " min")
lp("="^100)

# ---- gradient confirmation on the accepted starts ----
grads = Vector{Any}()
...
open(LOG_PATH, "w") do io
    ...
    println(io, "\n## Outcome\n")
    println(io, "**", length(accepted), " of ", N_STARTS, " common starts accepted** from ",
            n_cand, " candidate evaluations.\n")
    if length(accepted) < N_STARTS
        println(io, "> **INCOMPLETE**: fewer than ", N_STARTS, " starts were found within the budget ",
                "(max_candidates=", MAX_CANDIDATES, ", max_per_radius=", MAX_PER_RADIUS,
...
lp()
lp("="^100)
lp("DONE: ", length(accepted), "/", N_STARTS, " common starts; peak RSS ",
   round(peak_rss_mb(), digits = 0), " MB")
lp("="^100)
