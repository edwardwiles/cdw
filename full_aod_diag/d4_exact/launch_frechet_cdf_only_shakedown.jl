# CDF-only real D=20/W=80,000/L=50/:exclude_row outer shakedown launcher. Structural twin of
# launch_frechet_shakedown.jl (which stays :cdf_power, unchanged), using the threaded/syrk CDF-only
# value function and gradient (run_frechet_upper_cdf_only.jl, cm_frechet_cdf_only_gradient.jl).
# Task: FIXED_FRECHET_INNER_SOLVER production-feasibility 2026-07-24 (CDF-only addendum), task
# brief §12 (30-minute outer shakedown).
#
# Usage: julia launch_frechet_cdf_only_shakedown.jl <maxtime_real_seconds> [W] [L] [delta]
...
include(joinpath(@__DIR__, "run_frechet_upper_cdf_only.jl"))
...
const DELTA = length(ARGS) >= 4 ? parse(Float64, ARGS[4]) : 1.0

t0 = time()
lp("MAXTIME=$MAXTIME W=$W L=$L DELTA=$DELTA  nthreads=$(Threads.nthreads())  feature_set=:cdf_only")
...
base0, verify0 = cm_frechet_verified_state_threaded(vcat(gp_target, vec(exp.(pivot_expand(zfree_star, pe)))), fpcx, tls)
...
lp("[$(round(time()-t0,digits=1))s] launching run_frechet_upper_cdf_only, maxtime_real=$MAXTIME ...")
result = run_frechet_upper_cdf_only(fpcx, ctx, pe, w0, tls; delta = DELTA, maxtime_real = MAXTIME,
...
if result.best !== nothing
    ...
    base_cv, verify_cv = cm_frechet_verified_state_threaded(xf_best, fpcx, tls)
...
lp("progress per wall-clock minute:")
for (i, r) in enumerate(result.trace)
    r.t / 60 <= 1 || continue
end
n_by_minute = Dict{Int,Int}()
for r in result.trace
    m = Int(floor(r.t / 60)) + 1
    n_by_minute[m] = get(n_by_minute, m, 0) + 1
end
for m in sort(collect(keys(n_by_minute)))
    lp("  minute $m: $(n_by_minute[m]) evals (cumulative $(sum(n_by_minute[k] for k in 1:m if haskey(n_by_minute,k))))")
end
