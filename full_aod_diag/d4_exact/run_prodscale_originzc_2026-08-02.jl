...
# reusing run_coldsolve_originzc_w100k_2026-08-02.jl's exact template (incl. both real bug fixes:
# K_MEAN*D-length nu vector, k!-factorial nu targets) verbatim, only parametrizing W via ARGS[1]
# (defaults to 100_000) and adding a post-solve allocation measurement of the hot-path reduced FG
# functor (`base.st(x_state, g)`) and Hessian callback (`archA_partitioned_hess_cb_builder`). Must
# be launched as a FRESH `julia` process. Usage:
#   julia run_prodscale_originzc_2026-08-02.jl 500000
const D4X = @__DIR__
t0_total = time()
...
b_hess = try
    cb = archA_partitioned_hess_cb_builder(octx_reduced)
...
@printf("\nTOTAL WALL: %.2fs\n", time() - t0_total)
println("ORIGINZC_PRODSCALE_W$(W_VAL)_RESULT: ", ok ? "PASS" : "FAIL")
ok || exit(1)
