...
CKPT_DIR = joinpath(D4X_ROOT, "results", "profiled_ab_2026-08-01", "full_fixediter_gplow")
mkpath(CKPT_DIR)

println("="^90); println("FULL/REFERENCE ARM (fixed-iteration, gp=$(GP_FRAC)*calib): maxit=$MAXIT, gp0=$gp0"); println("="^90); flush(stdout)
t0 = time()
res = run_profile_checkpointed("full_fixediter_gplow", gp0, true, zfree0;
...
println("="^90)
@printf("RESULT full_fixediter_gplow: wall=%.1fs n_eval=%d n_grad_calls=%d native_outer_iters=%d\n",
    wall, res.n_eval, res.n_grad_calls, res.native_outer_diag.n_iters)
println("best=", res.best)
println("="^90)

outpath = joinpath(D4X_ROOT, "FULL_VS_PROFILED_OUTER_AB_FIXEDITER_GPLOW_2026-08-01_FULL_TRACE.csv")
