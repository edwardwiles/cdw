...
# over 5 waves. This runs ONE family, so the same core budget buys 5 starts per wave and the 10
# starts finish in 2 waves:
#
#            campaign (5 families)        this (PQ only)
#   per wave 2 starts x 5 fam = 10 jobs   5 starts x 1 fam = 5 jobs
#   cores    10 jobs x 10 = 100           5 jobs x 10 = 50        <- HALF the campaign's footprint
#   waves    5                            2
#
...
SEEDS=($(ls -d "$SEED_ROOT"/S* 2>/dev/null | sort -V))
if [ "${#SEEDS[@]}" -eq 0 ]; then log "ERROR: no seeds under $SEED_ROOT"; exit 2; fi
log "found ${#SEEDS[@]} seeds: $(basename -a "${SEEDS[@]}" | tr '\n' ' ')"
for s in "${SEEDS[@]}"; do
...
log "plan: ${#SEEDS[@]} starts / $STARTS_PER_WAVE per wave = $NWAVES waves"
log "      $(( STARTS_PER_WAVE * CORES_PER_PROC )) cores per wave; box has $(nproc)"
...
for ((w=0; w<NWAVES; w++)); do
    ...
    for ((i=lo; i<=hi; i++)); do
        ...
        odir="$CAMPAIGN_ROOT/$sid"; mkdir -p "$odir"
        c0=$(( FIRST_CORE + slot * CORES_PER_PROC )); c1=$(( c0 + CORES_PER_PROC - 1 ))
        log "  launching $sid on cores $c0-$c1 -> $odir/chain.log"
        taskset -c "$c0-$c1" julia --project=. \
            full_aod_diag/d4_exact/run_pq_multistart_seed_chain.jl \
            "$sdir" "$odir" "$PQ_L" "$DELTAS" > "$odir/chain.log" 2>&1 &
        pids+=($!); slot=$(( slot + 1 ))
    done
...
{
    echo "PQ-only multistart upper-bound campaign, L=$PQ_L, deltas=$DELTAS"
    ...
    echo
    echo "--- seed qualification under PQ (seeds were qualified for the OTHER five families) ---"
    for s in "${SEEDS[@]}"; do
        sid=$(basename "$s"); v="$CAMPAIGN_ROOT/$sid/seed_verdict.txt"
        [ -f "$v" ] && cat "$v" || echo "seed_id=$sid verdict=NO_VERDICT_FILE"
    done
    echo
    echo "--- per-chain results ---"
    for s in "${SEEDS[@]}"; do
        sid=$(basename "$s"); f="$CAMPAIGN_ROOT/$sid/chain_summary.txt"
        if [ -f "$f" ]; then sed "s/^/[$sid] /" "$f"; else echo "[$sid] (no chain_summary.txt)"; fi
    done
} > "$SUM"
log "summary: $SUM"
