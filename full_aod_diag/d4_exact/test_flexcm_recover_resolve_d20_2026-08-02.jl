...
# recover-then-resolve equivalence for FLEXIBLE CM's genuinely zero-dense reduced formulation.
# Direct flexible-CM analogue of test_zc_lane_originzc_recover_resolve_d20_2026-08-02.jl /
# test_zc_lane_cmzc_recover_resolve_d20_2026-08-02.jl (which validate this decisively for
# origin-ZC/CM+ZC) -- built here because no flexible-CM D20 gate exists yet against the NEW
# (2026-08-02) Reduced{CM}OperatorState zero-dense operator path (`reduced_cm_base_state`); prior
# flexible-CM D20 gates (e.g. test_flexcm_d20_gates.jl-style files) predate that reduced-operator
# work entirely.
#
# Combines, in ONE script:
#   STEP 1: REDUCED (genuinely zero-dense operator) inner solve at calibration, D20, W=W_VAL.
#   STEP 2: recover full-A from the reduced solve's own verified LFD.
#   STEP 3: re-solve the FULL (legacy, dense-column) flexible-CM problem at the recovered A.
#   STEP 4: decisive comparison (LFD/Delta_primal/Delta_dual match).
# W is read from ENV["FLEXCM_D20_W"] (defaults to 20000) so the same script drives both required
# points (W=20k and W=80k) without duplication.
const D4X = @__DIR__
...
const W_VAL = parse(Int, get(ENV, "FLEXCM_D20_W", "20000"))
...
const L_GRID = 10   # keeps the D20 solve tractable (ncm=(D-1)*L=190 at D=20); L=50 is exercised separately at production-dims W=100k
contrasts = :anchored

t_full = @elapsed begin
    aug_full = build_cm_augmented_obj_archB(ctx, CS; L = L_GRID, contrasts = contrasts)
...
t_reduced_build = @elapsed begin
    aug_reduced = build_cm_augmented_obj_archB(ctx, CS; L = L_GRID, contrasts = contrasts, base_obj = reduced_obj0, profiled_layout = layout)
...
println("cctx_reduced: NCORE=$(cctx_reduced.NCORE) ncm=$(cctx_reduced.ncm)")
flush(stdout)

println("\n" * "="^90); println("STEP 1: REDUCED (zero-dense operator) flexible-CM inner solve at calibration (D20, W=$W_VAL)"); println("="^90); flush(stdout)
reset_no_dense_g_counters!()
t_reduced = @elapsed base_reduced = reduced_cm_base_state(x_free_calib, ctx, layout, cctx_reduced)
...
# NOTE (bug found+fixed this session): verify_inner_solution_operator_cm! hardcodes
# `ncore1 = cf.oci-1` as the economic-block width, which is correct ONLY for the FULL/dense path
# where lambda's economic block literally has that width -- for the REDUCED path lambda's economic
# block width is layout.total_reduced_economic_moments (a DIFFERENT, smaller number; cf.oci-1 always
# describes the FULL economic moment space regardless of which family/layout built it), causing a
# length(lambda) != expected(ncore1+ncm) error. Use the SAME direct brute_force_verify (obj.moments!/
# Psi!/dPsi!) that test_zc_lane_cmzc_recover_resolve_d20_2026-08-02.jl already uses successfully for
# exactly this reason -- it reads obj.d directly (already reduced-width-correct) instead of cf.oci.
...
println("\n" * "="^90); println("STEP 3: re-solve the LEGACY FULL flexible-CM problem AT THE RECOVERED A"); println("="^90); flush(stdout)
t_full2 = @elapsed base_full2 = archC_base_state(x_free_recovered, ctx_cm_full, cctx_full)
