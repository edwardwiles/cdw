...
# 2026-08-11: OPTFILE/TAG let this same study run the inner-tolerance A/B through the REAL driver.
# The side harness (test_inner_opttol_ab) showed 1e-12 -> -102 and 1e-10 -> status 0 at this point,
# but it did NOT reproduce the driver's own result there, so its absolute numbers are not
# production-faithful. This runs the identical comparison through run_originzc_upper_checkpointed.
const OPTFILE = get(ENV, "OPTFILE", "ek_inner.opt")
const TAG = get(ENV, "TAG", "abort_cost_study_2026-08-11")
const OUT = joinpath(_D4E, "..", "..", "results", TAG)
...
INNER_KEEP_LAST_GOOD_X[] = true      # <-- the A/B knob for this run
lp("INNER_KEEP_LAST_GOOD_X = ", INNER_KEEP_LAST_GOOD_X[])
...
result = run_originzc_upper_checkpointed(w0; W = W, delta = DELTA, draw_design = :sobol_randomized,
    ...
    ckpt_dir = OUT, run_id = "abort_cost", label = "abort_cost",
