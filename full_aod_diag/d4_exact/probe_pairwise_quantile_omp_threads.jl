...
lp("="^96)
lp("OMP_NUM_THREADS PROBE: W=", W_AB, " L=", L_AB, " cutoff_source=:", CUTSRC)
lp("  opt file           = ", basename(first(OPTS)))
lp("  OMP_NUM_THREADS    = ", get(ENV, "OMP_NUM_THREADS", "<unset>"))
lp("  MKL_NUM_THREADS    = ", get(ENV, "MKL_NUM_THREADS", "<unset>"))
lp("  OPENBLAS_NUM_THREADS = ", get(ENV, "OPENBLAS_NUM_THREADS", "<unset>"))
lp("  JULIA_NUM_THREADS  = ", Threads.nthreads())
lp("="^96)

# Average cores actually used during a region: process CPU time (utime+stime from /proc/self/stat,
# fields 14/15, in clock ticks) divided by wall. This is what distinguishes "MKL is threaded and
# already saturating" from "MKL is pinned to one thread" -- wall-clock alone cannot.
const HZ = 100.0
function cpusecs()
    f = split(read("/proc/self/stat", String))
    return (parse(Float64, f[14]) + parse(Float64, f[15])) / HZ
end
...
ctx_cm = pcx.ctx_cm
ctx_cm.obj.inner_loop_opt = first(OPTS)
...
function one(label)
    ...
    wall = time() - t; cpu = cpusecs() - c0
    @printf("  %-22s wall=%8.2fs   cpu=%9.2fs   avg cores=%6.2f   n_fg=%d n_hess=%d   Delta=%.17g\n",
            label, wall, cpu, cpu/max(wall,1e-9), v.n_fg, v.n_hess, v.Delta_dual)
    return (wall=wall, cpu=cpu, cores=cpu/max(wall,1e-9), Delta=v.Delta_dual, cls=classify_inner_result(v))
end

lp("")
one("pass 1 (discarded)")
r = one("pass 2 (REPORTED)")
lp("")
@printf("RESULT  OMP_NUM_THREADS=%s  wall=%.2fs  avg_cores=%.2f  verified=%s\n",
        get(ENV,"OMP_NUM_THREADS","<unset>"), r.wall, r.cores, string(r.cls))
lp("="^96)
