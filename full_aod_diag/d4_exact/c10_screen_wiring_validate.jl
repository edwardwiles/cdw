...
function screen_and_check(xf, label)
    result, meta = evaluate_fullA_screened(xf, ctx; moment_representation = :dense, cache = nothing,
...
t_feas = @elapsed evaluate_fullA_screened(xf_feas, ctx; moment_representation = :dense, cache = nothing,
    use_cache = false, warm = false, pairwise = ctx.pairwise, witness = ctx.witness, use_witness = true)
...
for i in 1:20
    ...
    _, meta_try = evaluate_fullA_screened(xf_try, ctx; moment_representation = :dense, cache = nothing,
        use_cache = false, warm = false, pairwise = ctx.pairwise, witness = ctx.witness, use_witness = true)
...
if found_reject
    t_rej = @elapsed evaluate_fullA_screened(xf_rej, ctx; moment_representation = :dense, cache = nothing,
        use_cache = false, warm = false, pairwise = ctx.pairwise, witness = ctx.witness, use_witness = true)
