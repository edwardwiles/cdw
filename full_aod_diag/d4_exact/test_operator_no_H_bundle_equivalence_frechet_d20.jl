# ================================================================================================
# True no-H operator bundle task, Part A.7: common-Frechet real D=20/W=100,000 equivalence gate.
# This family's own documented history requires a real D=20 test (D=4-only is explicitly
# insufficient per this family's own prior investigations) -- this is that test.
# ================================================================================================
const _D4E = @__DIR__
...
println("="^90)
println("Common-Frechet D=20/W=100,000: operator-bundle (no H) vs dense-reference -- equivalence gate")
println("="^90)

pcx_d = build_cm_frechet_production_context(ctx, CS; L = 50, contrasts = :orthonormal, use_compressed_core = true,
                                             threaded_bins = true, cm_hessian_backend = :structured,
                                             inner_fg_backend = :cm_frechet_lookup, moment_representation = :dense_reference)
pcx_o = build_cm_frechet_production_context(ctx, CS; L = 50, contrasts = :orthonormal, use_compressed_core = true,
                                             threaded_bins = true, cm_hessian_backend = :structured,
...
function full_hessian_generic(ctx_cm, cctx, x::AbstractVector)
    ...
    hessian_cm_frechet_structured!(h, obj, cctx, level_targets)
...
println("="^90)
if isempty(FAILURES)
    println("ALL OPERATOR-VS-DENSE COMMON-FRECHET D=20/W=100000 EQUIVALENCE GATES PASSED")
else
