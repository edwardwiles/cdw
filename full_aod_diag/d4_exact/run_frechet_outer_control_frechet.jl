...
# marginal_restriction=:common_flexible is run_frechet_outer_control_frechet.jl -- run
...
CKPT_DIR = joinpath(D4X, "..", "..", "results", "fullA_d4", "frechet_outer_control_frechet")
...
println("="^90)
println("COMMON FRECHET control: delta=1 direct from calibration, real D=20/W=80000/L=10")
println("="^90)
t0 = time()
result = run_cm_upper_checkpointed(w0; W = W, delta = 1.0, draw_design = :pseudorandom, draw_seed = 20260719,
    ...
    marginal_restriction = :common_frechet, cm_gradient_backend = :cplus,
    ckpt_dir = CKPT_DIR, run_id = "frechet_control", label = "frechet_control",
    checkpoint_interval_s = 60.0, maxtime_real = 600.0, verbose = true)
wall = time() - t0

println()
println("="^90)
println("RESULT: common Frechet control")
