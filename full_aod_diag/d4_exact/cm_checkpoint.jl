...
function run_cm_upper_checkpointed(w0::Union{Nothing,Vector{Float64}} = nothing;
    ...
    function cb_G!(kc2, cb, evalRequest, evalResult, userParams)
        ...
        elseif is_frechet
            ...
            if cm_gradient_backend == :cplus
                # 2026-08-06: pass verify = verify_c too, matching the is_meanzc branch above --
                # cm_production_gradient_cplus now falls back to a fresh verified re-solve
                # whenever EITHER base OR verify is missing (see that function's own bug-fix
                # comment); passing the already-matched verify_c avoids a redundant re-solve on
                # the common (matched) case.
                cm_production_gradient_cplus(xf, pcx, ctx, pe, cplus_pool, cplus_ws; base = base, verify = verify_c, threaded = true,
                    h_mode = :cached, bandwidth_cache = bandwidth_cache)
            else
