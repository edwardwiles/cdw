# ============================================================================
# Same as run_fullA_D10.jl (D=10, μ FIXED, full A_od-in-outer-loop, γ_d≡1 +
# direct-γ' objective) but using PsiObjectiveBundleImplicitMethodBFullA
# instead of production PsiObjectiveBundleImplicit: eliminates the dense
# N×(d+2)×l Jacobian entirely, replacing it with the divergence-budget
# envelope scalar (Method B) + the trivial direct gravity gradient (both
# validated exact against production, relerr ~4e-15, in
# ad_benchmark/test_methodB_fullA2.log). Purpose: measure the REAL wall-time
# effect of removing the dense Jacobian from this framework, per user request.
#
#   julia --project=. full_aod_diag/run_fullA_D10_methodB.jl   (needs KNITRO env)
# ============================================================================
...
@assert D == D10
Aod_offset = 3 + D
println(">>> D=$D  l=$(length(θ_initial))  nTotalMoments=$nTotalMoments  outer_constr_index=$outer_constr_index  [METHOD B: no dense Jacobian]")
...
θ_lower_G[1] = θ_initial[1]; θ_upper_G[1] = θ_initial[1]   # μ FIXED
for d in 1:D
    idx = 2+d
...
println(">>> [RUN 3: full-A, METHOD B, MU_FIXED] free params=$nfree"); flush(stdout)

ggrav = CS.make_gravity_grad(γ, D)
mkobj(find_smallest, θi) = CS.PsiObjectiveBundleImplicitMethodBFullA(δ=1.0, find_smallest=find_smallest, γ=γ,
    ...
    lower_limit=-50, use_cached_x=true, gravity_grad=ggrav,
    outer_loop_opt=MAXIT_OPT, inner_loop_opt=INNEROPT)

t0=time()
println(">>> RUN3: solving for max(gamma'_focal) [-> kappa_lower]"); flush(stdout)
gp_max, θ_lo, st_lo, _ = outer_loop(mkobj(false, θ_initial_low_G), θ_lower_G, θ_upper_G, θ_initial_low_G)
println(">>> RUN3: solving for min(gamma'_focal) [-> kappa_upper]"); flush(stdout)
...
open(joinpath(OUT,"run3_fullA_D10_methodB_summary.txt"),"w") do f
    for ln in (
        "[RUN 3] full-A METHOD-B D=$D  baseIndex=$bi  free=$nfree  MU_FIXED=true  maxit=25",
...
@save joinpath(OUT,"run3_fullA_D10_methodB.jld2") κ_up κ_lo gp_max gp_min θ_up θ_lo st_up st_lo
println("RUN3 DONE")
