...
for every REDUCED row: confirmed (again, unchanged since [[full-vs-reduced-forensic-audit-2026-08-03]])
that no `CMZCFreeNuAdapter` or equivalent exists anywhere in the tree as of 2026-08-03.
...
const FAMILY_CAPABILITY_REGISTRY = Dict(
    ...
    _key(:unrestricted, _RED) => FamilyCapability(:unrestricted, _RED,
        ...
        "shared_family_outer_gradient", false, nothing,
        false, [:profiled_pivot_anchor_relative], false, false, false,
        "family_kind(fctx)=:unrestricted (same spelling as canonical); UnrestrictedFamilyCtx exists (profiled_family_adapters_2026-08-01.jl) but no D4/D20 gate exercising it through run_profiled_upper_constrained was found this session -- untested combination, not just unready. checkpoint/resume mechanism added to run_profiled_upper_constrained 2026-08-03 (task2 section5) and is family-agnostic, so it should apply here too, but this family has no gate of its own confirming that -- checkpoint_resume left false until one exists."),
    _key(:flexible_cm, _RED) => FamilyCapability(:flexible_cm, _RED,
        ...
        "shared_family_outer_gradient", false, nothing,
        false, [:profiled_pivot_anchor_relative], false, false, false,
        "REDUCED family_kind(fctx)=:flexible_CM (case differs from canonical :flexible_cm -- see REDUCED_FAMILY_KIND_TO_CANONICAL). Exercised by run_outer_flexcm_reduced_constrained_2026-08-02.jl. checkpoint/resume mechanism added to run_profiled_upper_constrained 2026-08-03 (task2 section5, verified for origin_zc) and is family-agnostic, but has no flexible_cm-specific gate confirming it -- checkpoint_resume left false until one exists."),
    _key(:common_frechet, _RED) => FamilyCapability(:common_frechet, _RED,
        ...
        "reduced verify_fn", "shared_family_outer_gradient", false, nothing,
        false, [:profiled_pivot_anchor_relative], false, false, false,
        "FrechetFamilyCtx exists (profiled_restricted_family_adapters_2026-08-02.jl); no dedicated run_outer_*_reduced_constrained script for this family found -- run_frechet_outer_control_frechet.jl exists but was not read this session, may be a different (older/unconstrained) driver. Verify before use. checkpoint/resume mechanism added to run_profiled_upper_constrained 2026-08-03 (task2 section5, verified for origin_zc) and is family-agnostic, but has no common_frechet-specific gate confirming it."),
    _key(:origin_zc, _RED) => FamilyCapability(:origin_zc, _RED,
        ...
        false, [:profiled_pivot_anchor_relative], false, false, false,
        "REDUCED family_kind(fctx)=:ZC_only. Exercised by run_outer_originzc_reduced_constrained_2026-08-02.jl. nu fixed at 1.0 for every origin (script's own comment) -- not free, unlike FULL's origin_zc row. checkpoint_resume verified live 2026-08-03 with a real D4 KNITRO round trip (18/18 checks, test_profiled_upper_constrained_checkpoint_resume_2026-08-03.jl) -- still production_ready=false: no W=100k cold/warm test, no fast-rejection test, no free-nu, no coordinate-mode formalization."),
    _key(:cm_meanzc, _RED) => FamilyCapability(:cm_meanzc, _RED,
        ...
        "reduced verify_fn", "shared_family_outer_gradient", false, nothing,
        false, [:profiled_pivot_anchor_relative], false, false, false,
        "REDUCED family_kind(fctx)=:CM_plus_ZC. CMZCFamilyCtx/evaluate_profiled_cmzc_point exist and are exercised by the D4 gate (test_phase13_production_runner_d4_gate_2026-08-02.jl) through the OLDER gp-fixed scaffold (profiled_production_outer_runner_2026-08-01.jl); also used (2026-08-03) as the deliberately-mismatched arm in a cross-family checkpoint-refusal test (test_profiled_upper_constrained_checkpoint_resume_2026-08-03.jl), which proves the namespace check rejects it correctly but does NOT exercise a real solve through run_profiled_upper_constrained for this family -- still unconfirmed."),
)
