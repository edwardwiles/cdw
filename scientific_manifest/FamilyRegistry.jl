...
const FAMILY_CAPABILITY_REGISTRY = Dict(
    ...
    _key(:origin_zc, _FULL) => FamilyCapability(:origin_zc, _FULL,
        ...
        "bin/run_profiled_model.jl FULL CLI wiring CLOSED 2026-08-04 (continuation): the earlier apparent " *
        "K_mean/K_pair conflict (campaign_cm_family_runner.jl K=1 vs campaign_cm_family_runner_sigma3.jl K=2, " *
        "both for :origin_specific_moments_zero_covariance) was a red herring -- neither file's restriction " *
        "TYPE is the one this CLI wrapper needed. Traced instead to run_outer_originzc_full_2026-08-02.jl, the " *
        "real dedicated FULL production script built specifically to match REDUCED's own OriginByPowerLayout(D,1,0) " *
        "-- distribution_restriction=:origin_specific_moments, K_mean=1, K_pair=0, matching this session's own " *
        "REDUCED-path free-nu/free-eta gates exactly. _run_full_originzc (bin/run_profiled_model.jl) real D20/W=20,000 " *
        "smoke: gp=0.9840278851786317 Delta=0.0045622715017980394 feasible=true verified=true, real checkpoint " *
        "files written."),
