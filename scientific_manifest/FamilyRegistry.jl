...
const FAMILY_CAPABILITY_REGISTRY = Dict(
    ...
    _key(:common_frechet, _RED) => FamilyCapability(:common_frechet, _RED,
        ...
        false, [:profiled_pivot_anchor_relative], false, false, false,
        "FrechetFamilyCtx exists (profiled_restricted_family_adapters_2026-08-02.jl); no dedicated run_outer_*_reduced_constrained script for this family found -- run_frechet_outer_control_frechet.jl exists but was not read this session, may be a different (older/unconstrained) driver. Verify before use. checkpoint_resume verified 2026-08-03 (test_all_family_checkpoint_resume_2026-08-03.jl, 52/52 checks) AND via a real D20/W=20,000 canonical CLI smoke (2026-08-04): status=-401, best gp=0.9746430740140182 Delta=0.10990155. Threaded HESSIAN parity (NOT the same field as threaded_outer_gradient -- left false, no direct evidence for that specific claim) confirmed at real D20/W=20,000 (2026-08-04): threaded==serial to ~1e-14, 3/3 fixed states. W=100,000 warm-start CONFIRMED (2026-08-04): exact status+Delta-star match cold vs warm (34.79s cold, warm completed). W=100,000 fast-rejection CONFIRMED (2026-08-04): additive log-A shift, fast at both W=20,000 (1.03s) and real W=100,000 (26.25s). D20+ outer-gradient FD gate still not run this session -- production_ready stays false."),
