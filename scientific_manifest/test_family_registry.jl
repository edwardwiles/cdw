...
@testset "FamilyRegistry" begin
    ...
    @testset "FULL rows are all checkpoint-resumable and production-ready; REDUCED rows are honestly not" begin
        for fam in CANONICAL_FAMILIES
            ...
            @test red_cap.production_ready == false   # section 12 bar not met by any REDUCED row yet
            @test red_cap.free_nu_supported == false   # confirmed zero free-nu implementations anywhere
        end
    end
    ...
    @testset "production_ready_families reflects the registry, not a hardcoded list" begin
        ...
        @test isempty(production_ready_families(:profiled_destination_scales))
    end
