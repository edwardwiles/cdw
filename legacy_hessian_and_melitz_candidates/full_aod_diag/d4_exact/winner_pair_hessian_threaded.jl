...
# each `@threads for tid in 1:nt` iteration (indexed by the LOOP VARIABLE,
...
function winner_pair_balanced_ranges(W::Int, nt::Int)
...
struct WinnerPairHessCtxThreaded
...
function build_winner_pair_ctx_threaded(cf::CompressedFactual; nt::Int = nthreads())
    base = build_winner_pair_ctx(cf)
    ncolI = base.ncolI
    ranges = winner_pair_balanced_ranges(base.W, nt)
    ...
    return WinnerPairHessCtxThreaded(base, nt, ranges, QQlocal, ulocal, rlocal,
        zeros(nt), zeros(nt), zeros(nt))
end

"""
    winner_pair_hessian_threaded!(h, obj, wctxt::WinnerPairHessCtxThreaded)
...
"""
function winner_pair_hessian_threaded!(h::AbstractVector, obj, wctxt::WinnerPairHessCtxThreaded)
    ...
    has_cf = wctx.has_cf
    length(h) == n * (n + 1) ÷ 2 || error("winner_pair_hessian_threaded!: length(h)=$(length(h)) != n(n+1)/2 for n=$n")
